...
bool SegmentLCTMatchBox::isMatched ( const CSCSegment &segment, std::vector<edm::Ref<std::vector<L1TMuon::TriggerPrimitive> > > CSCTFlcts,
				     int *match_report ){
  ...
  for ( auto t_lct = CSCTFlcts.cbegin(); t_lct != CSCTFlcts.cend(); t_lct++) {
    if (match_report)
      (*match_report) |= MATCH_CHAMBER;

    auto lct = *t_lct; // dereference the edm:Ref object
...
bool SegmentLCTMatchBox::isLCTAble ( const CSCSegment &segment, int *match_report ){
  ...
  for (hitIter = theHits.begin(); hitIter!= theHits.end(); hitIter++){
    ...
    if (_printLevel > 2 ){ // debug why this match didn't work
...
// to one of the CSCTF triggering tracks.  Actually, no it does not.  It returns true if Seg is matched to LCT only.
