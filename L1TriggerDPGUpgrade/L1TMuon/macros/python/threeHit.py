...
ptbin = [2, 3, 3, 4, 4, 5, 6, 7, 8, 9, 10, 12, 16, 20, 50, 90]
...
for iEvt in range(evt.GetEntries()):
    ...
    # 1-2-3  Mode 1 sum 3
    # Is the only one we care about right now
    ...
    # Which mode is track
    if temp_mode is 6: track_mode = 1

    if printLevel > 0: print ' This track is mode =', track_mode, '\n'
       
    if track_mode != 1: continue
       
    rpc_count['3hit_matched_mode1_trks'] += 1
    ...
    if evt.rpc_stat2_cluster_phi != -999:
        ...
        rpc_count['mode1_trks_w/rpc2_cluster'] += 1
        ...
        # Debug PtAddress.h problem
        #if evt.PtTrk_cluster_front[iCSCTrk] > 100: continue
